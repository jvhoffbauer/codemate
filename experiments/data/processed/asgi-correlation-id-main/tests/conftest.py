...
update_request_header_app = FastAPI(
    middleware=[Middleware(CorrelationIdMiddleware, update_request_header=True)]
)
...
transformer_app = FastAPI(
    middleware=[Middleware(CorrelationIdMiddleware, transformer=lambda a: a * 2)]
)
generator_app = FastAPI(
    middleware=[
        Middleware(CorrelationIdMiddleware, generator=lambda: TRANSFORMER_VALUE)
    ]
)
